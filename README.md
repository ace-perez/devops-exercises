...
A focused reference for interview preparation. Each question has a clean, digestible answer — key commands highlighted, fluff removed.
...
Linux is an open-source OS kernel created by Linus Torvalds in 1991. It dominates DevOps environments because:

- **Stability & security** — battle-tested in production at massive scale
- **CLI-first** — scripting and automation are native, not bolted on
- **Runs everything** — most servers, cloud platforms, Docker, and Kubernetes are Linux-based
- **Free & customisable** — install only what you need, reducing attack surface
- **Distributions** — Ubuntu, RHEL, CentOS, Debian etc. cover every use case
...
Permissions are split across three **types** and three **classes**:
...
So `chmod 754 file` sets those permissions numerically.
...
| Points to | Same inode as original | Path of the original file |
| Cross filesystem | ❌ No | ✅ Yes |
| If original deleted | Still works | Becomes broken |
| Same inode number | ✅ Yes | ❌ No |

**Hard link** = another name for the same data on disk.
...
A **process** is a running instance of a program with its own PID (Process ID) and memory space.
...
./script.sh &            # start a process in the background
```
...
| | Daemon | Regular Process |
|---|---|---|
| Started by | System at boot | User directly |
| Runs in | Background | Foreground (usually) |
| Has controlling terminal | ❌ No | ✅ Yes |
| Privileges | System-level | User-level |
| Lifetime | Continuous | Until task completes |
| Examples | `sshd`, `httpd`, `cron` | `vim`, `bash`, `python script.py` |
...
Environment variables are named values that affect how processes behave.
...
# Set for current shell only (not inherited by child processes)
MY_VAR="hello"
...
**SSH (Secure Shell)** is a protocol for encrypted remote access.

```bash
ssh user@hostname                    # basic connection
ssh-keygen                           # generate key pair
ssh-copy-id user@hostname            # deploy public key to server
```
...
- Restrict access with `AllowUsers` or `AllowGroups` in sshd_config
...
**systemd** is the init system and service manager used in most modern Linux distros.
...
**Unit files** live in `/etc/systemd/system/` and define service behaviour, dependencies, and startup conditions. After creating or editing a unit file:
```bash
systemctl daemon-reload        # register changes
```
...
free -h              # memory usage
vmstat 1 5           # CPU/memory/IO stats every 1s for 5 iterations
```
...
**Historical data:**
```bash
sar                  # system activity report (via sysstat)
```
...
Package managers handle **dependencies automatically** and verify package integrity.
...
**RAID (Redundant Array of Independent Disks)** combines disks for redundancy, performance, or both.

| Level | Name | How it works | Redundancy | Use case |
|---|---|---|---|---|
| RAID 0 | Striping | Data split across disks | ❌ None | Pure performance |
| RAID 1 | Mirroring | Data duplicated on 2+ disks | ✅ 1 disk failure | OS drives, critical data |
| RAID 5 | Striping + parity | Distributed parity across 3+ disks | ✅ 1 disk failure | Balanced storage |
| RAID 6 | Double parity | Like RAID 5 but 2 parity blocks | ✅ 2 disk failures | Large arrays |
| RAID 10 | Mirror + stripe | Mirrors then stripes | ✅ 1 per mirror pair | High performance + redundancy |
...
3. **Logical Volumes (LVs)** — what you actually format and mount

**Why it's useful:**
- Resize volumes **without downtime**
- Span a volume across multiple disks
- Take **snapshots** for backups
- Migrate data between disks while online

**Key commands:**
```bash
pvcreate /dev/sdb              # create physical volume
vgcreate myvg /dev/sdb         # create volume group
lvcreate -L 50G -n data myvg   # create logical volume
lvextend -L +20G /dev/myvg/data  # grow a volume
resize2fs /dev/myvg/data         # resize the filesystem after extending
pvdisplay / vgdisplay / lvdisplay  # view info
```
...
`/etc/fstab` defines which filesystems are mounted automatically at boot and how.
...
| Options | `defaults`, `ro`, `noatime`, `noexec` |
| Dump | 0=no backup, 1=include in dump |
| Fsck | 0=skip, 1=root, 2=other filesystems |

**Use UUIDs** (`blkid` to find them) rather than `/dev/sdb1` — device paths can change, UUIDs don't.
...
# 3. Test gateway
ip route show           # find your gateway
ping <gateway IP>
...
# 6. Check routing
traceroute 8.8.8.8
mtr 8.8.8.8             # better than traceroute

# 7. Check if a port is listening/open
ss -tulnp | grep 80
nc -zv hostname 443
...
⚠️ Always allow SSH **before** enabling the firewall to avoid locking yourself out.
...
Both are **Mandatory Access Control (MAC)** systems — they restrict what processes can do, even if running as root.

| | SELinux | AppArmor |
|---|---|---|
| Used by | Red Hat, CentOS, Fedora | Ubuntu, SUSE |
| Approach | Security contexts (labels) | File path profiles |
| Complexity | Higher | Lower |
| Modes | Enforcing / Permissive / Disabled | Enforce / Complain |

**SELinux commands:**
```bash
sestatus                    # check status and mode
...
adduser username            # interactive version (Debian)
...
**Sudo:** Configure with `visudo` — never edit `/etc/sudoers` directly.

**Account expiry:**
```bash
chage -l username           # view expiry settings
...
**CPU:**
```bash
top / htop / mpstat         # identify bottlenecks
nice / renice               # adjust process priority
taskset -c 0,1 ./app        # pin process to specific cores
...
# Tune dirty page flushing
sysctl -w vm.dirty_ratio=15
sysctl -w vm.dirty_background_ratio=5
```

**Disk I/O:**
- Use `XFS` for large files, `ext4` for general workloads
- Mount with `noatime` to skip timestamp updates on reads
- Choose the right I/O scheduler: `deadline` for SSDs, `cfq` for mixed HDDs
...
Set resource limits in `/etc/security/limits.conf` for applications with high file or process counts.
...
**Simple rsyslog forwarding:**
```bash
# On clients — add to /etc/rsyslog.conf:
*.* @logserver:514          # UDP
*.* @@logserver:514         # TCP (more reliable)

# On the log server — enable receiving:
module(load="imudp")
...
**Core tools:**
```bash
rsync -avz --delete /source/ /backup/    # incremental file sync
tar -czf backup.tar.gz /etc             # archive
dd if=/dev/sda of=/backup/disk.img      # full disk image
```

**Databases — use specialised tools:**
```bash
mysqldump -u root -p mydb > backup.sql  # MySQL logical backup
...
- Set retention policies (daily/weekly/monthly)
...
1. **Minimise attack surface** — remove unused packages and services
2. **User security** — principle of least privilege, sudo over root, strong passwords
3. **SSH hardening** — key auth only, disable root login, change port
4. **Firewall** — whitelist only required ports
5. **MAC** — enable SELinux or AppArmor
6. **Patch management** — keep system updated, enable automatic security updates
7. **Intrusion detection** — install `fail2ban` to block brute force
8. **Auditing** — enable `auditd`, review logs regularly
9. **File integrity** — monitor critical files with tools like AIDE
10. **Mount options** — use `noexec`, `nosuid` on `/tmp`, `/var`
...
echo "net.ipv4.ip_forward=1" >> /etc/sysctl.conf  # set permanently
...
**Load at boot** — add module name to `/etc/modules`
...
**Set module parameters permanently** — create `/etc/modprobe.d/mymodule.conf`:
```
...
ps aux --sort=-%cpu | head -10      # sorted list
perf top                            # profiling
```
...
vmstat 1 5                          # watch swapping activity
cat /proc/meminfo                   # detailed breakdown
journalctl -k | grep -i "oom\|killed"  # OOM kills
```
...
df -h                               # is a partition full?
du -sh /* | sort -rh | head         # find large directories
smartctl -a /dev/sda                # check disk health
```

Once identified — remediation could be optimising application config, adjusting resource limits, or replacing hardware.
...
**Layer 7 (Application level) — HAProxy / Nginx:**
```bash
...
**Layer 4 (Transport level) — Linux Virtual Server (LVS) with Keepalived:**
- Higher throughput, less overhead
- IP-level routing, no content inspection

**Algorithms:**
- `roundrobin` — distribute equally
- `leastconn` — send to least-loaded server
- `source` — same client always hits same server (sticky sessions)

For **HA of the load balancer itself**, use Keepalived with VRRP to provide a floating virtual IP that fails over between two load balancers.
...
Containers package an application with its dependencies, sharing the host OS kernel rather than running a full OS. Lighter than VMs, faster to start, and consistent across environments.
...
docker stop / rm <container_id>     # stop and remove
...
Configuration management automates consistent setup across many servers.

| Tool | Model | Language | Agent |
|---|---|---|---|
| Ansible | Push | YAML | None (SSH) |
| Puppet | Pull | DSL | Yes |
| Chef | Pull | Ruby | Yes |
| SaltStack | Push/Pull | YAML | Optional |
...
**Best practices:** version control everything, test in staging first, use roles for reusability.
...
Software RAID uses the kernel's `md` driver managed by `mdadm`:

```bash
# Create RAID 1 (mirror) across two disks
mdadm --create /dev/md0 --level=1 --raid-devices=2 /dev/sda1 /dev/sdb1

# Create RAID 5 across three disks
...
# Physically replace disk, partition it
...
Quotas limit disk usage per user or group:
...
# 3. Initialise quota database
quotacheck -cum /home

# 4. Enable quotas
quotaon -v /home

# 5. Set limits for a user
edquota -u username
# Set soft limit (warning), hard limit (enforcement), grace period

# 6. View usage
repquota -a          # all filesystems
quota -u username    # specific user
```
...
`systemd-networkd` manages network interfaces on systemd-based systems. Config files live in `/etc/systemd/network/`.
...
**Static IP config — `/etc/systemd/network/10-eth0.network`:**
```ini
[Match]
Name=eth0
...
networkctl status                    # view interface status
```
...
Namespaces isolate system resources for processes — they're the foundation of container technologies like Docker.
...
**Commands:**
```bash
unshare --net bash          # start shell in new network namespace
nsenter --pid --net -t <pid>  # enter existing namespace of a process
lsns                         # list namespaces
```

When Docker starts a container, it creates a new set of namespaces so the container has its own isolated view of PIDs, network, and filesystem.
...
`journald` collects logs from all systemd services in a structured binary format, with metadata like unit name, priority, and timestamps built in.
...
**Make logs persistent** (default is in-memory only):
```bash
...
```bash
timedatectl status                   # view current time, timezone, NTP status
timedatectl set-timezone Europe/Dublin
timedatectl set-ntp true             # enable NTP sync

# Check chrony (most common NTP client)
chronyc tracking                     # sync status and offset
chronyc sources                      # which NTP servers are in use
systemctl status chronyd             # service status
```

**Configure NTP servers** in `/etc/chrony/chrony.conf`:
```
server 0.pool.ntp.org iburst
server 1.pool.ntp.org iburst
```

**Why it matters for fleet ops:** clock drift breaks log correlation, TLS certificate validation, Kafka ordering, and distributed consensus. Alerts on time sync failure should be in every monitoring setup.
...
**cgroups (Control Groups)** limit and account for resource usage of process groups — CPU, memory, disk I/O, and network.

Docker and Kubernetes use cgroups under the hood to enforce container limits.

```bash
# Set limits via systemd (recommended approach)
systemctl set-property nginx.service CPUQuota=20%
...
**Direct cgroup manipulation (advanced):**
```bash
...
`tc` (Traffic Control) shapes and prioritises network traffic at the kernel level — useful for bandwidth limiting, QoS, and simulating network conditions in testing.

```bash
# Limit bandwidth on eth0 to 1Mbit/s
...
**Key concepts:**
- **qdisc** — queuing discipline (how packets are queued)
- **HTB** — Hierarchical Token Bucket, for class-based rate limiting
...
**Common HA stack: Pacemaker + Corosync**
...
**Key components:**
- **Corosync** — cluster messaging layer (node communication)
- **Pacemaker** — cluster resource manager (what runs where)
- **STONITH** — fencing (forcibly kill a failed node to prevent split-brain)
...
**Rule:** Always test failover regularly. An untested HA cluster is not HA.
...
apt install netfilter-persistent    # auto-restore on boot
```

**Troubleshooting:**
```bash
# Is iptables the issue? Temporarily flush all rules
iptables -F

# Add logging before a DROP rule to see what's being blocked
iptables -A INPUT -j LOG --log-prefix "BLOCKED: "
iptables -A INPUT -j DROP

# Count hits per rule
iptables -L -v -n     # look at the pkts column
```
...
**Configure logging level** in `/etc/rsyslog.conf`:
```
kern.warning    /var/log/kern.log
*.err           /var/log/errors.log
*.* @@logserver:514          # forward to central server (TCP)
```

**Log rotation** — configure in `/etc/logrotate.d/myapp`:
```
/var/log/myapp/*.log {
    weekly
    rotate 4
...
- Ship logs off-host immediately (can't tamper with what's already gone)
- Alert on critical patterns (OOM kills, authentication failures, service crashes)
- Establish baselines so anomalies stand out
...
# Scrub (verify data consistency — schedule this periodically)
echo check > /sys/block/md0/md/sync_action

# Replace a failed disk
mdadm /dev/md0 --fail /dev/sdb1
mdadm /dev/md0 --remove /dev/sdb1
# Physically swap disk, repartition to match
mdadm /dev/md0 --add /dev/sdb1
```
...
journalctl -u <service> --since today  # today only

# Debug a unit file
systemd-analyze verify /etc/systemd/system/myservice.service

# Override settings without editing the original
systemctl edit <service>               # creates drop-in file
systemctl daemon-reload                # apply after any changes
```
...
```bash
vm.swappiness = 10             # reduce swapping — critical for DBs
vm.dirty_ratio = 15
vm.dirty_background_ratio = 5
fs.file-max = 100000           # handle high connection counts
net.core.somaxconn = 1024
```

**Memory:** Allocate ~70-80% of RAM to the DB buffer pool (MySQL: `innodb_buffer_pool_size`, PostgreSQL: `shared_buffers`).

**Storage:**
- Use `XFS` — scales well and handles concurrent I/O
- Mount with `noatime,nodiratime`
- Separate volumes for data, logs, and temp files (prevents I/O contention)
...
numactl --localalloc            # bind process to local NUMA memory
```
...
| | LXC | Docker |
|---|---|---|
| Design | System containers (like a lightweight VM) | Application containers (one process per container) |
| Runs | Full OS with init system | Single application process |
| Image format | None — configured directly | Layered image format (Dockerfile) |
| Registry | No standard registry | Docker Hub and others |
| Tooling | `lxc-create`, `lxc-start`, LXD | `docker build`, `docker run`, Docker Compose |
| Best for | Multi-process, VM-like isolation | Microservices, CI/CD, app portability |
| Runtime | Uses kernel directly | containerd (originally LXC-based) |

Both use Linux **namespaces** and **cgroups** under the hood.

Use LXC when you need VM-like behaviour without the VM overhead. Use Docker when you're packaging and deploying applications.
...
**Boot sequence: firmware → GRUB → kernel → initramfs → systemd**

**Won't POST / no boot:**
- Check hardware: power, RAM seating, cables
- Listen for beep codes, check BMC/IPMI for hardware errors

**GRUB issues:**
```bash
# At GRUB menu — press 'e' to edit boot entry
# Add to kernel line for single-user mode:
systemd.unit=rescue.target
# Or add 'single' for legacy mode
```

**Kernel panic:**
```bash
journalctl -b -1                  # logs from previous boot
dmesg | grep -i "panic\|error"
```

**Initramfs issues:**
```bash
# From rescue/live USB, chroot in and regenerate:
...
```bash
fsck /dev/sda1                   # from rescue environment, unmounted
```
...
**rsyslog** is the most common syslog daemon. Config in `/etc/rsyslog.conf`.
...
**Forward to a central log server:**
```bash
# UDP (faster, lossy)
*.* @192.168.1.100:514
...
**Modular config** — drop files in `/etc/rsyslog.d/`:
```bash
echo '*.* @@logserver:514' > /etc/rsyslog.d/50-forward.conf
...
- TLS for remote log transmission
...
1. **Power on → POST** — firmware (BIOS/UEFI) checks hardware
2. **Boot device selection** — BIOS reads MBR / UEFI reads GPT to find bootloader
3. **GRUB** — loads the kernel and initramfs into memory
4. **Kernel initialisation** — kernel decompresses, detects hardware, mounts initramfs
5. **initramfs** — temporary root filesystem with just enough to find the real root
6. **Real root mounted** — kernel mounts the actual root filesystem (`/`)
7. **systemd (PID 1)** — first real process, reads unit files and starts services
8. **Target reached** — `multi-user.target` or `graphical.target`
...
| Crash isolation | Process crash is isolated | Thread crash can kill all threads |
| Context switch | Slow | Fast |

A process is an independent execution environment. Threads are lighter units of execution within a process that share the same memory.
...
**D state** is notable — a process stuck here is usually waiting on a disk or NFS operation. `kill -9` won't work until the I/O resolves.

```bash
ps aux | awk '$8 == "D"'    # find processes in D state
```
...
A **zombie** is a process that has finished executing but its entry remains in the process table because its parent hasn't collected its exit status (`wait()` syscall).
...
**Fix:** Restart the parent process. If the parent is already gone, the zombie will be adopted by init (systemd) and cleaned up.
...
- **Virtual memory** — each process sees its own address space (isolated from others)
- **Paging** — memory divided into 4KB pages, mapped between virtual and physical via page tables
- **Demand paging** — pages only loaded into RAM when accessed, not upfront
- **Copy-on-write (COW)** — after `fork()`, child shares parent's pages until it writes, then gets its own copy
- **Page cache** — kernel caches disk reads in RAM for performance
- **Swap** — when RAM is exhausted, least-used pages moved to disk

```bash
free -h                     # RAM and swap usage
cat /proc/meminfo           # detailed breakdown
vmstat 1 5                  # track paging/swapping activity
```
...
A **kernel panic** is Linux's equivalent of a crash — the kernel detects an unrecoverable error and halts.

**Common causes:**
- Hardware failure (bad RAM, failing disk)
- Corrupt filesystem
- Bad or incompatible kernel module
- Kernel bug
...
/var/crash/                          # crash dumps (if kdump configured)
last reboot                          # when did it happen?
```

**Configure automatic reboot after panic:**
```bash
# In /etc/sysctl.conf:
kernel.panic = 30         # reboot 30s after panic
```
...
When the system runs out of memory and can't swap, the **OOM (Out Of Memory) killer** steps in and kills a process to free memory.

**How it chooses:** each process has an `oom_score` (0-1000) — higher score = more likely to be killed. Factors include memory usage and time running.
...
# 3. Check for configuration errors (most services have a test mode)
nginx -t
...
# 7. Check SELinux/AppArmor denials
ausearch -m AVC -ts recent

# 8. Validate unit file syntax
systemd-analyze verify /etc/systemd/system/<service>.service

# 9. Try running the binary directly
/usr/sbin/nginx -g "daemon off;"    # run outside systemd to see raw error output
```
